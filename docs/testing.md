...
The bridge suite includes cross-endpoint crypto interoperability, shared bridge/watch pairing vectors, persistent and concurrent replay rejection, timestamp bounds, stale-terminal replay rejection, and a negative matrix proving that reasoning/item/raw-response notifications cannot be classified as a finished turn. Relay tests cover mutual endpoint proof state, substitution rejection, pre-allocation admission, source rate limiting, bounded pending state, idempotent ciphertext queues, acknowledgement, refusal to persist offline watch audio, and suppression of duplicate Firebase wakes. Watch tests verify atomic alert claiming, foreground suppression of pre-existing alerts, a ten-second fast transcription retrieval window, an idle voice state after leaving transcript review, one continuous one-second vibration policy, and silence-gated voice activity mapping.
