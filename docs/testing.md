...
Inspect `home`, `home-alert`, `home-update`, `home-update-downloading`, `home-update-ready`, `home-listening`, `home-speaking`, `home-transcribing`, `pair`, `sessions`, `transcript`, `send-mode`, `reply`, `text-reply`, `chat`, `chat-idle`, `chat-permission`, `chat-error`, `approval`, `complete`, `error`, and `settings` on a round Wear OS emulator. Check clipping, crown-scroll reachability, touch targets, notification icon rendering, one-second alert vibration semantics, and motion at normal and disabled animation scales. The home session button must remain readable with long titles, the two- and three-action button groups must stay inside the circular safe area with at least 48 dp targets, and update availability must use the dedicated compact action without adding a fourth dock button. The layered voice control must remain the primary focal point and retain its cyan/violet edge light, recessed inner well, and legibility at compact square size. Listening and transcribing must expose a broad, continuously fading coral cancellation field without changing the inner control action; the normal inner control and all other buttons keep their actions, while tapping the surrounding field or otherwise empty active-screen background cancels immediately. The listening state must remain still under the voice noise gate. The speaking state must retain its inner level-driven waveform. Transcribing must retain the fixed open-ring progress glow and show a live elapsed time. The transcript demo must retain the frozen elapsed time in review. Live chat must show both roles, preserve headings/emphasis/lists/code, smoothly collapse and expand `UPDATE` cards, keep final answers fully rendered, expose like/dislike controls for completed Codex responses, distinguish `PERMISSION` cards and their controls, show actionable multi-line failures, and keep terminal notifications enabled while visible. While a session is active without an unresolved permission or error, show the compact thinking indicator and keep it followed at the latest message; permission waits must never present as thinking. Its round-screen Reply action must open a round-safe Voice/Text chooser, and Text must open the Wear keyboard, preserve the exact selected task, and send through the same reviewed model, effort, and queue/steer path as voice. Verify the Settings opt-out leaves update cards expanded.
...
Use only these synthetic states for external visual review. Send the primary, transcript, session, approval, completion, and error screenshots to Gemini 3.7 Flash at High reasoning in Antigravity, iterate on actionable findings, and stop at that review gate if Gemini is unavailable.
