...
When a turn is active, a normal Watch Send reads Codex Desktop's current `followUpQueueMode`; the Watch therefore follows the same queue-or-steer default as the other Codex clients. A long-press on Send exposes one-shot Queue and Steer overrides, carried as `followUpAction` and preserved if the encrypted submission must be retried. Steer updates a Watch-controlled turn through `thread/settings/update` plus the exact active-turn precondition in `turn/steer`; an Android, iOS, or Desktop-controlled turn is steered through Desktop's signed `send_message_to_thread` interface with the original thread ID plus the selected model and reasoning. Because an inference cannot swap models after it starts, the active turn keeps its original model while the Watch selection becomes the chat's sticky configuration for the next new turn. Queue uses the App Server queue immediately when the selected settings already match; a differing selection remains durably pending until the active turn finishes so it can start exactly once with the requested model and reasoning. If no turn is active, the bridge uses the existing queue/start path. None of these routes creates or forks another chat.

The bridge registers a tiny relay as a Codex Desktop-managed MCP process. That process exposes no model tools; it only brokers the authenticated local same-chat call from the background service through Desktop's native signed tool pipe. Before dispatch, the bridge seals the exact pending submission plus the IDs of the recent user messages in the mode-0600 bridge config. It removes the record only after Desktop acknowledges the follow-up. If the bridge exits or loses the acknowledgement, it first checks the original chat for a new matching user message; it reports the handoff as complete when found and waits through a bounded propagation window before any retry. Existing pre-handoff pending records migrate through this path automatically. If the signed active-turn route is temporarily unavailable, the encrypted prompt stays pending rather than silently changing into a later queued turn. No fallback creates or forks another chat. Bounded history and `thread/queue/list` remain observation-only and never mark a `notLoaded` session `Ready`; green readiness requires a loaded idle session or proof that the exact Watch submission was accepted. Internal App Server cancellation task IDs are retried during request synchronization and suppressed for foreign terminal scans; a controlled cancellation is reported only as a generic interrupted result. See [Realtime voice foundation](realtime-voice.md) for the separately diagnostic-only experimental realtime route.
