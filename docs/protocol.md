...
For an idle `notLoaded` session, the bridge first tries to resume the original thread with the selected model and a scoped reasoning-effort config, then repeats those values through `thread/settings/update` before adding or starting the prompt. If another client retains the writer and its persisted model/effort already match the Watch selection, the bridge can queue immediately on that same chat. If the selections differ, it does not add a queue item that could auto-start under the wrong configuration. Instead it encrypts the exact pending submission in the mode-0600 bridge config, acknowledges `waiting`, and retries the original thread after ownership releases. A later `turn.started` status tells the Watch when the requested selection has been applied. Pending submissions survive bridge restarts and retain the same idempotency key; they never create or fork another chat.

A genuinely busy foreign session is never resumed or interrupted. Matching settings may queue behind its active turn; differing settings remain encrypted and waiting until the session can be acquired safely. Bounded history and `thread/queue/list` are observation-only and never mark a `notLoaded` session `Ready`; green readiness requires a loaded idle session or proof that the bridge started the exact Watch submission. Internal App Server cancellation task IDs are retried during request synchronization and suppressed for foreign terminal scans; a controlled cancellation is reported only as a generic interrupted result. See [Realtime voice foundation](realtime-voice.md) for the separately diagnostic-only experimental realtime route.
