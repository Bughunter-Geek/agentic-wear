...
`turn.accepted` is a transaction acknowledgement: the watch retains the editable draft until App Server has accepted the matching request ID and the prompt is either running or safely queued for a foreign owner. For an idle `notLoaded` session, the bridge checks that no turn is currently running and adds the prompt with the Watch request UUID as its idempotency key. If no other client retains the writer, it resumes the thread, applies the selected model and effort, and calls `thread/queue/start` for that exact queued submission. It records the returned turn ID and releases the subscription only when that turn reaches a terminal state, allowing iOS and Android to continue afterward. If Desktop or mobile still retains the idle writer, the bridge leaves the accepted queue item intact for that owner rather than deleting it or stealing the session. A genuinely busy foreign session is never resumed or interrupted; its owning client receives the queued submission after the active turn. A successful bounded history and queue probe marks that session `Ready` for the Watch; merely listed sessions remain `Available`. See [Realtime voice foundation](realtime-voice.md) for the separately diagnostic-only experimental realtime route.
